    ...
    <div class="flex-1 flex flex-col lg:flex-row min-h-screen">
        ...
        <div class="lg:w-1/2 flex flex-col justify-between p-8 sm:p-12 lg:p-16 bg-gradient-to-br from-emerald-800 via-teal-900 to-slate-950 text-white relative overflow-hidden border-b lg:border-b-0 lg:border-r border-slate-200 dark:border-slate-800/80">
            ...
            <div class="grid grid-cols-3 gap-4 pt-6 text-center relative z-10 border-t border-emerald-500/20">
                <div class="flex flex-col items-center">
                    ...
                    <span class="text-xs font-semibold text-slate-900 dark:text-white">Encrypted</span>
                ...
                <div class="flex flex-col items-center">
                    ...
                    <span class="text-xs font-semibold text-slate-900 dark:text-white">Safe Audit</span>
                ...
                <div class="flex flex-col items-center">
                    ...
                    <span class="text-xs font-semibold text-slate-900 dark:text-white">Fast Reset</span>
        ...
        <div class="lg:w-1/2 flex items-center justify-center p-6 sm:p-12 lg:p-16 bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100">
            <div class="w-full max-w-md bg-white dark:bg-slate-900/90 rounded-3xl shadow-2xl  p-8 sm:p-10 border border-slate-200 dark:border-slate-800 backdrop-blur-sm">
                <div>
                    <h2 class="text-2xl sm:text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight">Forgot Password</h2>
                    <p class="mt-1.5 text-xs sm:text-sm text-slate-500 dark:text-slate-400">Enter your email address and we'll help you reset your password.</p>
                </div>
                    ...
                    <div class="mt-4 rounded-xl bg-emerald-950/60 border border-emerald-500/40 p-4 text-xs text-emerald-200 space-y-2">
                        ...
                        @if(session('reset_url'))
                            <div class="pt-2 border-t border-emerald-500/30">
                                <span class="block text-[11px] text-slate-500 dark:text-slate-400 font-mono mb-1">Local Testing Link:</span>
                                <a href="{{ session('reset_url') }}" class="block truncate rounded-lg bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100 p-2 text-[11px] font-mono text-emerald-400 underline hover:text-emerald-300">
                ...
                <form action="{{ route('password.email') }}" method="POST" class="mt-6 space-y-4" novalidate>
                    ...
                    <div>
                        <label for="email" class="block text-xs font-semibold text-slate-700 dark:text-slate-300 mb-1.5">Registered Email <span class="text-rose-400">*</span></label>
                        <div class="relative rounded-xl border @error('email') border-rose-500 /50 @else border-slate-200 dark:border-slate-800 @enderror bg-slate-50 dark:bg-slate-950 text-slate-800 dark:text-slate-100/80 focus-within:border-emerald-500 focus-within:ring-1 focus-within:ring-emerald-500 transition">
                            <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3.5 text-slate-500 dark:text-slate-400">
                                <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                                </svg>
                            </div>
                            <input id="email" name="email" type="email" required value="{{ old('email') }}" placeholder="your@email.com" class="block w-full rounded-xl bg-transparent py-2.5 pl-10 pr-3 text-sm text-slate-900 dark:text-white placeholder-slate-400 focus:outline-none">
                        </div>
                        @error('email')
