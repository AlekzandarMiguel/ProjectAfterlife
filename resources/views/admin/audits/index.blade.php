...
<div class="space-y-6">
    <div class="pb-4 border-b border-slate-800">
        <h2 class="text-lg font-bold text-white tracking-tight">System Audit Log Ledger</h2>
        <p class="text-xs text-slate-400 mt-0.5">Tamper-evident record of all critical system actions, status transitions, and authentication events.</p>
    </div>
            ...
            <tbody class="divide-y divide-slate-800 font-mono text-[11px]">
                @foreach($logs as $log)
                    <tr class="hover:bg-slate-900/60 transition">
                        ...
                        <td class="px-6 py-3.5 text-slate-200">{{ $log->user->name ?? 'System / Anonymous' }}</td>
                        <td class="px-6 py-3.5 text-slate-400 truncate max-w-[200px]">{{ $log->entity_type ? class_basename($log->entity_type) . ' #' . $log->entity_id : '—' }}</td>
                        <td class="px-6 py-3.5 text-slate-400">{{ $log->ip_address }}</td>
                        <td class="px-6 py-3.5 text-slate-400">{{ $log->created_at->format('Y-m-d H:i:s') }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
