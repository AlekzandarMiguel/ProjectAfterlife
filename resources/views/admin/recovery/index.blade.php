...
<div class="space-y-6">
    <div class="pb-4 border-b border-slate-200 dark:border-slate-800">
        <h2 class="text-lg font-bold text-slate-900 dark:text-white tracking-tight">Recovery Health & Inactivity Monitor</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Track developer activity on adopted projects, identify stale workspaces, and manage inactive or re-abandoned software.</p>
    </div>

    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        @foreach($projects as $project)
            <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white/40 dark:bg-slate-900/40 p-5 flex flex-col justify-between space-y-4">
                <div>
                    ...
                    <div class="mt-2 text-xs text-slate-500 dark:text-slate-400">
                        Owner: <span class="text-slate-800 dark:text-slate-200 font-medium">{{ $project->owner->name }}</span>
                    </div>
                    ...
                    @endphp
                    <div class="mt-4">
                        <div class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 mb-1 font-mono">
                            <span>Progress</span>
                    ...
                    @if($project->isUnderRecovery())
                        <div class="grid grid-cols-2 gap-2">
                            <form action="{{ route('admin.recovery.inactive', $project) }}" method="POST">
                                @csrf
                                <button type="submit" class="w-full rounded bg-slate-100 dark:bg-slate-800 border border-slate-300 dark:border-slate-700 px-2 py-1 text-[11px] text-slate-700 dark:text-slate-300 hover:bg-slate-700">
                                    Mark Inactive
                                </button>
                            </form>
