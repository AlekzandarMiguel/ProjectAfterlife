...
<div class="space-y-6">
    <div class="flex flex-col sm:flex-row sm:items-center justify-between gap-4 pb-4 border-b border-slate-800">
        <div>
            <h2 class="text-lg font-bold text-white tracking-tight">User Directory & Verification</h2>
    ...
    <div class="flex items-center justify-between gap-4 pb-2">
        ...
        <div class="flex items-center gap-2">
            ...
            <a href="{{ route('admin.users.index', ['status' => 'pending']) }}" class="px-3 py-1.5 rounded-lg text-xs font-medium inline-flex items-center gap-1.5 {{ request('status') === 'pending' ? 'bg-amber-600 text-white shadow-lg shadow-amber-900/30' : 'bg-slate-800 text-slate-400 hover:text-white' }} transition">
                <span>Pending Verification</span>
                @if(isset($pendingCount) && $pendingCount > 0)
                    <span class="px-1.5 py-0.2 rounded-full bg-amber-400 text-slate-950 font-bold text-[10px]">{{ $pendingCount }}</span>
                @endif
            </a>
            <a href="{{ route('admin.users.index', ['status' => 'active']) }}" class="px-3 py-1.5 rounded-lg text-xs font-medium {{ request('status') === 'active' ? 'bg-emerald-600 text-white shadow-lg shadow-emerald-900/30' : 'bg-slate-800 text-slate-400 hover:text-white' }} transition">
                Active
            </a>
            <a href="{{ route('admin.users.index', ['status' => 'suspended']) }}" class="px-3 py-1.5 rounded-lg text-xs font-medium {{ request('status') === 'suspended' ? 'bg-rose-600 text-white shadow-lg shadow-rose-900/30' : 'bg-slate-800 text-slate-400 hover:text-white' }} transition">
                Suspended
            </a>
        </div>
    </div>
        ...
        <table class="min-w-full divide-y divide-slate-800 text-left text-xs text-slate-300">
            ...
            <tbody class="divide-y divide-slate-800">
                @forelse($users as $user)
                    <tr class="hover:bg-slate-900/60 transition {{ $user->isPending() ? 'bg-amber-950/10' : '' }}">
                        <td class="px-6 py-4 flex items-center gap-3">
                            <div class="w-8 h-8 rounded-full bg-slate-800 border border-slate-700 flex items-center justify-center font-bold text-white text-xs shrink-0">
                            ...
                            <div>
                                ...
                                <div class="text-[10px] text-slate-400 font-mono">{{ $user->email }} • {{ '@' . $user->username }}</div>
                            </div>
                        </td>
                        <td class="px-6 py-4">
