...
<div class="space-y-6">
    <div class="pb-4 border-b border-slate-200 dark:border-slate-800">
        <h2 class="text-lg font-bold text-slate-900 dark:text-white tracking-tight">My Adoption Requests</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Track the status of your submitted adoption proposals.</p>
    </div>
            ...
            @foreach($requests as $req)
                <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white/40 dark:bg-slate-900/40 p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                    <div class="space-y-1">
                        ...
                        <div class="text-xs text-slate-500 dark:text-slate-400">Target completion: {{ $req->expected_completion_date->format('M d, Y') }} • Submitted {{ $req->created_at->diffForHumans() }}</div>
                        @if($req->admin_notes)
                            <div class="mt-2 text-xs text-slate-700 dark:text-slate-300 bg-slate-50/60 dark:bg-slate-950/60 p-2.5 rounded-lg border border-slate-200 dark:border-slate-800">
                                <span class="font-semibold text-slate-800 dark:text-slate-200">Admin Notes:</span> {{ $req->admin_notes }}
                            </div>
                        @endif
                    </div>
                    ...
                    <div class="flex items-center gap-3 shrink-0">
                        ...
                        @endif
                        <a href="{{ route('user.adoptions.show', $req) }}" class="rounded-lg border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950 px-3.5 py-2 text-xs text-slate-700 dark:text-slate-300 hover:text-white transition">
                            View Details
                        </a>
                    </div>
                </div>
    ...
    @else
        <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white/30 dark:bg-slate-900/30 p-12 text-center">
            <p class="text-xs text-slate-500 dark:text-slate-400">You have not submitted any adoption requests yet.</p>
