...
<div class="space-y-6">
    <div class="pb-4 border-b border-slate-200 dark:border-slate-800">
        <h2 class="text-lg font-bold text-slate-900 dark:text-white tracking-tight">Active Recovery Workspaces</h2>
        <p class="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Projects currently under your ownership being repaired and updated.</p>
    </div>

    @if($projects->count() > 0)
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            @foreach($projects as $project)
                <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white/40 dark:bg-slate-900/40 p-6 flex flex-col justify-between hover:border-slate-300 dark:border-slate-700 transition">
                    <div>
                        ...
                        <p class="mt-2 text-xs text-slate-500 dark:text-slate-400 line-clamp-2 leading-relaxed">{{ $project->short_description }}</p>
                        ...
                        @endphp
                        <div class="mt-6">
                            <div class="flex items-center justify-between text-xs text-slate-500 dark:text-slate-400 mb-1.5 font-mono">
                                <span>Recovery Progress</span>
                                <span class="font-bold text-slate-900 dark:text-white">{{ $progress }}%</span>
                            </div>
                            ...
                            <div class="text-[10px] text-slate-500 dark:text-slate-400 font-mono mt-1">{{ $completed }} of {{ $total }} recovery tasks completed</div>
                        </div>
                    </div>
    ...
    @else
        <div class="rounded-xl border border-slate-200 dark:border-slate-800 bg-white/30 dark:bg-slate-900/30 p-12 text-center">
            <h3 class="text-base font-semibold text-slate-900 dark:text-white">No active recovery workspaces</h3>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">You do not currently own any projects under active recovery.</p>
